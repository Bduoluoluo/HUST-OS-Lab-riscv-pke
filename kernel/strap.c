  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
      ...
      if (stval < USER_STACK_TOP) {
        pte_t *pte = page_walk((pagetable_t)current->pagetable, ROUNDDOWN(stval, PGSIZE), 0);
        if (pte && (*pte & PTE_RSW_0)) {
          void* child_pa = alloc_page();
          uint64 pa = PTE2PA(*pte);
          memcpy(child_pa, (void*) pa, PGSIZE);
          *pte ^= PTE_V | PTE_RSW_0;
          user_vm_map((pagetable_t)current->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, (uint64)child_pa,
