...
static int kernel_page_counter = 0;
...
int s_start(void) {
  ...
  if (cpu_id == 0) pmm_init();

  // build the kernel page table
  if (cpu_id == 0) kern_vm_init();

  sync_barrier(&kernel_page_counter, NCPU);
